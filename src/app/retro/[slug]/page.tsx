import { getPostBySlug, getRetroPosts, getLatestPosts } from "@/lib/data";
...
export default async function RetroArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const post = getPostBySlug(slug);
  const relatedNews = getRetroPosts().filter(n => n.id !== post?.id).slice(0, 3);
  
  if (!post || post.type !== 'retro') {
    notFound();
  }
      ...
      <header className="container mx-auto px-4 lg:px-8 mb-12 max-w-5xl">
        <div className="flex items-center gap-2 mb-6">
          ...
          {post.tags?.map(t => (
            <Link 
              key={t} 
              href={`/retro?filter=${encodeURIComponent(t)}`}
