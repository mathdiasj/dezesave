...
function RetroFilterContent({ posts }: { posts: Post[] }) {
  ...
  const filters = useMemo(() => {
    const defaultFilters = ["Todos", "Nostalgia", "N64", "GBA", "RPG", "Aventura"];
      ...
      <div className="flex flex-col gap-8">
        {filteredPosts.length > 0 ? (
          ...
          <div className="py-12 text-center text-gray-500 font-inter">
            Nenhum clássico encontrado para o filtro &quot;{activeFilter}&quot;.
          </div>
        )}
      </div>
    </>
...
export function RetroFilter({ posts }: { posts: Post[] }) {
  return (
    <Suspense fallback={
      ...
      <RetroFilterContent posts={posts} />
    </Suspense>
  );
}
